/**
 * TestererfassungDialog — pre-generated create/edit dialog for Testererfassung.
 *
 ...
 * TestererfassungDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Testererfassung['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<TestererfassungDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Testererfassung, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Testererfassung';
...
/** Widened prefill type for TestererfassungDialog.defaultValues — see file header. */
export type TestererfassungDialogDefaults = Omit<Testererfassung['fields'], 'status'> & {
...
interface TestererfassungDialogProps {
  ...
  defaultValues?: TestererfassungDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  status: LOOKUP_OPTIONS['testererfassung']?.['status'] ?? [],
};
...
export function TestererfassungDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: TestererfassungDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Testererfassung['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'testererfassung');
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('testererfassung') })
    : t('new_entity', { entity: appLabel('testererfassung') });
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'status': (
      <div key="status" className="space-y-1.5">
        <Label htmlFor="status">{fieldLabel('testererfassung', 'status')}</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('testererfassung', 'status', 'offen') ?? 'Offen'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('testererfassung', 'status', 'in_bearbeitung') ?? 'In Bearbeitung'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('testererfassung', 'status', 'abgeschlossen') ?? 'Abgeschlossen'}
          </button>
        </div>
      </div>
    ),
    ...
    'bestanden': (
      <div key="bestanden" className="space-y-1.5">
        <Label htmlFor="bestanden">{fieldLabel('testererfassung', 'bestanden')}</Label>
        <div className="flex items-center gap-2 pt-1">
          ...
          />
          <Label htmlFor="bestanden" className="font-normal">{fieldLabel('testererfassung', 'bestanden')}</Label>
        </div>
      </div>
    ),
  };
  ...
  const FIELD_LABELS: Record<string, string> = {"titel": "Titel", "beschreibung": "Beschreibung", "datum": "Datum", "status": "Status", "ergebnis": "Ergebnis", "bestanden": "Test bestanden"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.TESTERERFASSUNG} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
