...
export class LivingAppsService {
  ...
  static async getTestererfassungEntry(id: string): Promise<Testererfassung | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Testererfassung;
    return hydrateRecords([record], 'testererfassung')[0];
  }
  static async createTestererfassungEntry(fields: CreateTestererfassung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.TESTERERFASSUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'testererfassung') });
    return { ...data, record_id: data.id };
  }
  static async updateTestererfassungEntry(id: string, fields: Partial<CreateTestererfassung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.TESTERERFASSUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'testererfassung') });
    return { ...data, record_id: data.id };
  }
