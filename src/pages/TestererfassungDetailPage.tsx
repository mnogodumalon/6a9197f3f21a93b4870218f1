...
import { formEnhancements } from '@/config/form-enhancements/Testererfassung';
import { evalComputed } from '@/config/form-enhancements/types';
import { t, appLabel, fieldLabel, localeTag, CURRENCY } from '@/i18n';

export default function TestererfassungDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [record, setRecord] = useState<Testererfassung | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const list = await LivingAppsService.getTestererfassung();
  ...
  async function handleUpdate(fields: Testererfassung['fields']) {
    if (!record) return;
    await LivingAppsService.updateTestererfassungEntry(record.record_id, fields);
    await loadData();
    setEditing(false);
  }
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteTestererfassungEntry(record.record_id);
    setDeleteOpen(false);
    navigate('/testererfassung');
  }
  ...
  if (!record) {
    return (
      <RecordViewEmpty
        title={t('not_found')}
        action={
          <Button variant="ghost" onClick={() => navigate('/testererfassung')}>
  ...
  return (
    <RecordView
      onBack={() => navigate('/testererfassung')}
      onEdit={() => setEditing(true)}
      backLabel={t('back')}
      editLabel={t('edit_button')}
    >
      <RecordHeader title={record.fields.titel ?? appLabel('testererfassung')} />
