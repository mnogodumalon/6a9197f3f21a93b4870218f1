...
const listers: Record<EntityKey, () => Promise<RawRecord[]>> = {
  'testererfassung': () => LivingAppsService.getTestererfassung() as Promise<RawRecord[]>,
};
...
const queriers: Record<EntityKey, (q: RecordQuery) => Promise<RawRecord[]>> = {
  'testererfassung': q => LivingAppsService.queryTestererfassung(q) as Promise<RawRecord[]>,
};

const counters: Record<EntityKey, (filter?: string, signal?: AbortSignal) => Promise<number>> = {
  'testererfassung': (filter, signal) => LivingAppsService.countTestererfassung(filter, signal),
};

const creators: Record<EntityKey, (fields: Record<string, unknown>) => Promise<RawMutation>> = {
  'testererfassung': fields => LivingAppsService.createTestererfassungEntry(fields as never),
};
